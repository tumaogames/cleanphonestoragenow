...
/// - Chooses best-fit columns/rows or obeys fixed constraints
/// - Alignment: Start/Center/End/Justify per axis
/// - Order: RowMajor / ColumnMajor, with horizontal/vertical directions & reverse
/// - Pixel snapping, optional child resizing
/// - Works with ScrollRect (AllowVerticalScroll)
/// </summary>
[ExecuteAlways]
...
public class FluidGridLayout : LayoutGroup
{
    ...
    public enum OverflowMode { ClampToParent, AllowVerticalScroll }
    ...
    [Tooltip("If true, enforces the given aspect (width/height). Ignored if keepSquare = true.")]
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        ComputeLayout();
        // Expose preferred width to parent for size fitters
    ...
    {
        // Expose preferred height to parent for size fitters
    ...
    private void ComputeLayout()
    {
        ...
        // Determine candidate columns/rows based on constraint
        ...
        void Consider(int cols)
        {
            ...
            // Horizontal fit → candidate width
            float totalSpacingX = spacing.x * Mathf.Max(0, cols - 1);
            float cw = (parentW - totalSpacingX) / cols;

            // Vertical fit → candidate height
            float ch;
            if (overflow == OverflowMode.AllowVerticalScroll)
            {
                // Fit width perfectly; height derived from width or clamped (since we can scroll)
                if (keepSquare) ch = cw;
            ...
            float cellH;
            if (keepSquare) cellH = cellW;
            else if (enforceAspect) cellH = Mathf.Clamp(cellW / Mathf.Max(0.01f, aspectWH), minCell.y, maxCell.y);
            else cellH = Mathf.Clamp(ch, minCell.y, maxCell.y);
            ...
            if (overflow == OverflowMode.ClampToParent)
            {
                ...
                fits &= usedW <= parentW + eps;
                fits &= usedH <= parentH + eps;
            }
            if (!fits) return;

            // Score: prefer larger area; tie-breaker fewer rows then fewer cols
        ...
        // Run search based on constraint mode
        switch (constraint)
        {
            ...
            default:
                ...
                {
                    // Fallback: at least try minCols
                    Consider(minCols);
                }
                break;
        }

        // Hard fallback if still invalid (tiny rect or extreme constraints)
        if (bestScore == float.NegativeInfinity)
        {
    ...
    private void ApplyChildrenLayout()
    {
        ...
        {
            case HAlign.Center: startX = padding.left + Mathf.Max(0, (parentW - usedW) * 0.5f); break;
            case HAlign.End: startX = padding.left + Mathf.Max(0, parentW - usedW); break;
            case HAlign.Justify:
                // base start is left; spacingX adjusted below
                break;
        }
        if (centerX) startX = padding.left + Mathf.Max(0, (parentW - usedW) * 0.5f);

        // Vertical start
        float startY = padding.top;
        switch (verticalAlign)
        {
            case VAlign.Center: startY = padding.top + Mathf.Max(0, (parentH - usedH) * 0.5f); break;
            case VAlign.End: startY = padding.top + Mathf.Max(0, parentH - usedH); break;
            case VAlign.Justify:
                // base start is top; spacingY adjusted below
                break;
        }
        if (centerY) startY = padding.top + Mathf.Max(0, (parentH - usedH) * 0.5f);

        // Adjust spacing for Justify (distribute the remaining space)
        float spacingX = spacing.x;
        float spacingY = spacing.y;

        if (horizontalAlign == HAlign.Justify && _chosenColumns > 1 && parentW > usedW)
        {
            float freeW = Mathf.Max(0, parentW - (_chosenColumns * _chosenCell.x));
            spacingX = freeW / Mathf.Max(1, _chosenColumns - 1);
        }
        if (verticalAlign == VAlign.Justify && _chosenRows > 1 && parentH > usedH)
        {
        ...
        // Layout children in desired order
        ...
        for (int rawIndex = 0; rawIndex < rectTransform.childCount; rawIndex++)
        {
            ...
            // Map displayIndex to row/col based on order mode
            int col, row;
            ...
            {
                // Position only, keep child's current size
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        // Compute top-left corner in local space for drawing preview
        float usedW = _chosenColumns * _chosenCell.x + spacing.x * Mathf.Max(0, _chosenColumns - 1);
        float usedH = _chosenRows * _chosenCell.y + spacing.y * Mathf.Max(0, _chosenRows - 1);
        ...
        for (int row = 0; row < _chosenRows; row++)
        {
            for (int col = 0; col < _chosenColumns; col++)
            {
                ...
                // Draw as wire cubes for preview
