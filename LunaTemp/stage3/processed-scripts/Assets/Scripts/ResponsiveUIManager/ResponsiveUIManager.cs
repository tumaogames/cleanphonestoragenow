...
public sealed class ResponsiveUIManager : MonoBehaviour
{
    ...
    public Color debugTextShadow = new Color(0, 0, 0, 0.5f);
    ...
    void ApplyAll(bool force = false)
    {
        ...
        if (_canvas == null || _scaler == null) TryCache();
        ...
        if (manageCanvasScaler && _scaler)
        {
            ...
            if (enableDpiClamp && dpi > dpiClampThreshold)
                effectiveDpi = Mathf.Min(dpi, dpiClampMax);

            // Your DPP formula (visual density)
            float dpp = Mathf.Clamp(1f * Mathf.Max(1f, effectiveDpi / 326f), 1f, 2.25f);

            if (!Mathf.Approximately(_lastDpp, dpp))
            {
                // Try CanvasScaler.dynamicPixelsPerUnit via reflection (works across runtimes)
                bool set = TrySetDynamicPixelsPerUnit(_scaler, dpp);

                if (!set)
                {
                    // Fallback: adjust referencePixelsPerUnit (effective crispness)
                    float baseline = Mathf.Max(1f, _scaler.referencePixelsPerUnit);
                    _scaler.referencePixelsPerUnit = Mathf.Clamp(baseline * (dpp / 1f), 50f, 400f);
                }
    ...
    // Reflection-based setter to avoid compile errors when the API isn't present on target runtime
    bool TrySetDynamicPixelsPerUnit(CanvasScaler scaler, float value)
    {
        try
        {
            var prop = typeof(CanvasScaler).GetProperty("dynamicPixelsPerUnit");
            if (prop != null && prop.CanWrite)
            {
                prop.SetValue(scaler, value, null);
                return true;
            }
        }
        catch { /* ignore */ }
